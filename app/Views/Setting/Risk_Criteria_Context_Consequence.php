<script>
    ...
    function generateTabsAndTables($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $html .= '<table id="example1" class="table table-hover mt-3">';
            $html .= '<thead>';
            $html .= '<tr>';
...
<script>
    function load_modal(check, check_type, data_encode) {
        ...
        modal_risk_criteria_consequence = document.getElementById("modal_risk_criteria_consequence");
        $(".modal-body #iss").empty();

        if (check == '1') {
            //--show modal requirment--//
            console.log('Showing modal 1');;
            modal_risk_criteria_consequence.style.display = "block";
        }
    }
</script>
<div class="modal fade" id="modal-default">
    <div id="modal_risk_criteria_consequence">
        <?= $this->include("Modal/Risk_Criteria_Context_Consequence"); ?>
    </div>
</div>
