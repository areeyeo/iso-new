                    ...
                    <div class="card-body">
                        ...
                        <div class="" id="risklevelmaxtrix" style="overflow-x:auto;">
                            <table class="table table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width: 130px; background-color: #658ABF;color: floralwhite; font-weight: 500;"
                                            class="text-center" colspan="<?= count($Likelihood_level_context) ?>">
                                            <div>ผลกระทบ</div>
                                ...
                                <tbody>
                                    <tr>
                                        <th rowspan="<?= count($Likelihood_level_context) + 1 ?>"
                                        ...
                                        <tr>
                                            ...
                                            <?php foreach ($Likelihood_level_context as $impact): ?>
                                                ...
                                                <td class="text-center"
                                                    style="background-color: <?= getRiskColor($result, $Risk_level_context) ?>; color: <?= getTextColor($result, $Risk_level_context) ?>">
                                                    <?= $result ?>
                                                </td>
                                            <?php endforeach; ?>
                                        </tr>
                                        ...
                                        <tr>
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu"
                                                        ...
                                                        <li><a class="dropdown-item"
                                                                onclick="confirm_Alert('Do you want to delete risk level <?= $risk['risk_leve'] ?> ?', 'planning/risk_Criteria_Context_Risk_Level/delete/<?= $risk['id_risk_level_context'] ?>')">Delete</a>
                                                        </li>
                                                        <li>
                                            ...
                                            <td>
                                                <input type="radio" id="radio_<?= $key ?>" name="riskAssessment"
                                                    ...
                                                    onclick="change_assessment_level('planning/risk_Criteria_Context_Risk_Level/change_assessment_level/<?= $risk['id_risk_level_context'] ?>')">
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
...
<script>
    function load_modal(check, check_type, data_encode) {
        ...
        var risk_level_data = <?php echo json_encode($Risk_level_context); ?>;

        if (check == '1') {
            //--show modal risk Level--//
            modal_crud_criteria_risk_level.style.display = "block";
            $(".modal-body #url_route").val("planning/risk_Criteria_Context_Risk_Level/create");
        } else if (check == '2') {
            modal_crud_criteria_risk_level.style.display = "block";
            $(".modal-body #risklevel").val(risk_level_data[check_type]['risk_leve']);
            $(".modal-body #riskcolor").val(risk_level_data[check_type]['risk_color']);
            $(".modal-body #textcolor").val(risk_level_data[check_type]['text_color']);
            $(".modal-body #minranges").val(risk_level_data[check_type]['minimum']);
            $(".modal-body #maxranges").val(risk_level_data[check_type]['maximum']);
            $(".modal-body #description").val(risk_level_data[check_type]['description']);
            $(".modal-body #url_route").val("planning/risk_Criteria_Context_Risk_Level/edit/" + risk_level_data[check_type]['id_risk_level_context']);
        }
    }
</script>
<script>
